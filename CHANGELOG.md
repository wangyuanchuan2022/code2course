...
- **理解骨架六件（进硬下限与机检）**：① **仓库总架构图** `.arch-scene`（封面，与调用图并存；三视图合一——对象从属层次 / 一次运行的数据流与**回边** / 模块归属标注 `module_marks`）② **一次完整运行链路** `.run-chain`（封面，对象级时序 ≥4 步）③ **模块三句话卡** `.module-card`（四行：解决什么问题/输入/输出/在总架构图上的位置）④ **变量词典** `.vardict-scene`（`代码变量 | 人话 | 生命周期 | 代码位置` + 变量生命周期链 `.var-chain`，v1.18.0 起从工作笔记升为**交付物**）⑤ **设计四问块** `.design-qa`（它做什么／为什么这么做／**不这么做会怎样**／**为什么不用更简单的方法**；每模块 ≥2 处、L3 ≥3 处，四问缺一不算）⑥ **改造指南** `.upgrade-guide`（结业段，常见修改任务 → 去哪儿改，≥6 行真实任务）。
- **`validate_course.py` 检查 19–23**（与既有 18 项并列，缺位即 ERROR，与检查 17 同口径）：19 封面总架构图与运行链路 ／ 20 每模块三句话卡（`.mc-row` ≥4）／ 21 变量词典与生命周期链 ／ 22 设计四问块计数与四问完整（按档位 1/2/3）／ 23 结业改造指南（`.ug-row` ≥4，L2/L3 ≥6）。**既有 18 项检查语义与文案未改**；检查 16 的键集白名单作**兼容性扩容**（节点 `desc`/`role`、边 `kind`/`detail`），并注册 `arch-data` 数据块（`.arch-data` 顶层额外允许 `module_marks`）。
- **resources：总架构图复用调用图引擎**（`app.js` §20 由"调用图专用"升级为"两宿主共用引擎"）：参数化 `cgLayout`/`cgRenderScene` + 两组显式选项；节点 `desc`/`role`/`view`、边 `kind`/`detail`；**箭头承载 kind**（owns 无头／call·dependency 实心／data 空心／control 圆点），**线型仍只承载置信度**；`.arch-legend` 图例、模块归属带 + `.arch-marks` 行；导出 `window.c2cCallgraphLayout`/`window.c2cArchLayout` 供课程脚本与测试复用。五件静态组件用既有 token 写样式（**不新增动画引擎、不新增状态类、不引入新色**）。
- **`tests/test_render_arch.mjs`**（零依赖 node harness，51 断言）：最小 DOM stub + `node:vm` 加载**生产 app.js 本体**渲染，架构图三次渲染逐字节一致；既有 `.callgraph-scene` 缺省路径**逐字节冻结基线**（sha256 钉值）+ 闸门注入必红自证（改 `CG_NODE_H` 即红）。
- **流程与验收条款**：workflow §1c「逻辑架构梳理」（总架构图与运行链路的事实底稿）、§2.5 由三张表扩为**四张表**（变量词典入交付）、§6「交互克制原则与复杂度护栏」、§8.1「交付报告与自评」（十维度自评表 + 交互组件总数/产物行数记录）、§10 修订复跑清单增"理解骨架完整"；quality-gates 新增陷阱 40–44 与对应验收条。

### Changed

- **视觉覆盖扩容 + 交互下限取消**：承载理解的静态件（架构图/结构图解/三句话卡/变量词典/生命周期图/设计四问块/改造指南）**计入视觉与深度元素**；每模块"交互元素种类 ≥2"的硬下限**改为按需**——交互数量不再是达标手段；装饰性交互判定沿用"删掉测试"但**改判向：动效不达标不再等于要补动效，而是把篇幅让给架构与设计**。
- **调用图/总架构图语义契约升级**：节点增 `desc`（它做什么）/`role`（系统里的角色），边增 `kind`（owns/call/dependency/data/control）/`detail`（怎么依赖、传什么），悬停事实面板渲染这些字段——**诚实性通道（出处与置信度线型）不被语义通道挤占**。
- **结业段由三件升为四件**（增改造指南）；**使命定位上移**：SKILL.md 与 README 明确"终点不是读者看懂了，而是接手者敢动手改"（评审者原话）；audience.md 增「读者动作面」。
- **隐喻预算（S3）**：同模块隐喻 ≤3；隐喻数 ≥ 该模块核心变量数一半时**必须**以变量词典替代/压减隐喻；每个隐喻须登记映射到的具体变量或函数（不得只映射抽象概念）。

### Known limitations

- **example 未重建**（按批次任务书留下批）：三件套已变，示例课件内联外壳过期；旧成品在检查 19–23 下**预期不达标**（这正是本批要暴露的差距），课程重做批次产出后回填。
- 变量生命周期图（A4）的机检以检查 21 的 `.var-chain`（≥2 节点）**间接覆盖**，未单列检查编号；隐喻预算（S3）与交互克制（S4）为文本层条款，**无机检**。
- 【独立终验登记·P2】检查 23（与既有 `finale_check` 同形，非本批引入）对「无结业段」的课程**真空通过**：删掉结业段后 rc=0/0 错，汇总行 `改造指南 0 个 0 行 ✓` 的 ✓ 语义是「无错误」而非「已满足」；处置待下批二选一——无结业段时该件标 `—`（不适用），或先入契约再加「课程须含结业段」独立检查。
- 【独立终验登记·P2】`module_marks` 覆盖完备性未机检：现行检查 19 只要求「≥2 正式模块时非空」，不要求每个正式模块都被某 mark 覆盖。
- 【独立终验登记·P2/P3】README「发布检查项」缺迁移提示——**检查 19–23 是常开门**，既有课件只换新外壳不补理解骨架六件，跑 validate 必得 14 错（属预期差距非回归）；callgraph 数据块的节点 4–20 语义字段仍为文本层要求（与 arch-data 机检不对称）；validate 负向变异体实数 23 + 2 档位门（批次文案「21 个」为计数口径差）。
- 浏览器视觉终检（无头 0 pageerror + 交互冒烟）与对比度矩阵扩表未纳入本批：本批无真实课程使用新组件，确定性由 node harness 逐字节证明，对比度由独立探针覆盖新组件 11 对 × 双主题（全 ≥4.5）。
- `course-template.html` 顶部注释保留字面 `<script>` 字样（HEAD 即有，属 HTML 骨架自身，不参与内联）。
