...
> 📌 本节是 `resources/base.css` §1 的**快照**（v1.11.2），仅供理解变量语义；
...
### 8a. 调用图（`.callgraph-scene`，v1.13.1，base.css §20）

调用图是"结构事实"的图，不是手绘示意图——配色同样沿用第 3 节语义色，**不引入新色**：

| 元素 | 颜色 | 说明 |
|---|---|---|
| 节点盒描边 | `--border`（默认）/ `--accent`（聚焦、高亮） | 空心描边风格，与站点 hero 图同款克制 |
| 节点盒底 | `--bg-card`（默认）/ `--accent-soft`（高亮，配 `--text` 深字） | 高亮态即 `is-cg-hot` |
| 类型 glyph | `--accent-text`（entry）/ `--accent-2`（class · module）/ `--text-muted`（其余） | 分组色只从既有 token 里取 |
| `verified` 连线 | `--text` 实线 | 实锤：调用行可查 |
| `inferred` 连线 | `--text-muted` 虚线 | 推断：命名或导入推测 |
| 回边连线 | `--accent-3` 虚线 | 指向画面上方的环边 |
| 事实面板 | `--text-muted` + `--font-mono` | 与 data-viz 详情行同族 |

约定：

- **线型承载诚实性，颜色不承载**——实锤与推断的区别只能靠实线/虚线，不得改用两种颜色（颜色一换就会被读成"两类东西"，而它们其实是同一件事的两种确定度）
- 高亮一律走 `--accent` 族，**不用松绿**：绿色在本站点里已被探照灯/赌注的"点亮代码行"占用，混用会让"这两处高亮是不是同一件事"变得无法回答
- 节点数建议 4–20、边数 ≤ 2 倍节点数；超过就该拆成两张图或改用探照灯逐段走读
...
**动画属性分级（v1.11.2 成文）**：持续/循环/高频动画必须走 `transform` / `opacity`（合成层，不触发布局）；**一次性入场动画允许 `width` / `height` 等布局属性**（参考实现：`.viz-bar` 生长动画，滚入视口仅播一次、条数少，且 `scaleX` 方案会缩放圆角）——这是显式例外，不是惯例，自定义动画优先合成属性。

**通用布局小件（v1.11.2）**：`.tag-row`（标签横排，封面技术栈/模块头用，替代对 `.tp-head` 的挪用）、`.hero-meta`（封面规模与受众行）、`.course-footer`（出品信息页脚）。类名单一语义：`.tp-head` 仅限翻译块头。
